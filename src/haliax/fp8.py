...
class OverwriteWithGradient(eqx.Module):
    ...
    FP8. This module is a marker that indicates to [hax.quant.apply_updates] that the
    gradient should be used to overwrite the state rather than added to it.
    """
    pass


class Fp8DotGeneralOp(eqx.Module, OverwriteWithGradient):
    ...
    # copied from flax
    def __call__(self, lhs, rhs, dimension_numbers, precision: PrecisionLike = None, preferred_element_type: DTypeLike | None = None):
        ...
        x_qdq = in_qdq(
            comp_dtype, lhs, self.input_scale, self.input_amax_history
        )
        k_qdq = in_qdq(
            comp_dtype, rhs, self.kernel_scale, self.kernel_amax_history
        )
